...
fn main() -> Result<(), Box<dyn error::Error>> { // Box<dyn Error> - means any error
    ...
    file2.read_to_string(&mut file2_content).expect("Couldn't read to string");
    println!("{}", file2_content);
    Ok(())
}


fn read_file() -> Result<String, Error> { // signifies that it will return Ok(string) or Error
...
fn read_file_shorter() -> Result<String, Error> {
    let mut content = String::new();
...
use std::fs;
fn read_file_megashort() -> Result<String, Error> {
    fs::read_to_string("hello.txt")
}

// ? operator works for Result<>(returns an error) and Option<> (returns a None)
fn get_last_num() -> Option<i32> {
    let nums = vec![32, 2, 11];
